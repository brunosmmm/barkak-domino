...
from .cpu import create_cpu_player
...
class GameRoomManager:
    ...
    def create_game(self, request: CreateGameRequest) -> tuple[Game, Player]:
        ...
        for i in range(cpu_count):
            cpu_player = create_cpu_player(i)
    ...
    def add_cpu_player(self, game_id: str, player_id: str) -> tuple[bool, str, bool]:
        ...
        # Count existing CPUs to get unique names
        cpu_count = sum(1 for p in game.players if p.is_cpu)
        cpu_player = create_cpu_player(cpu_count)
        game.players.append(cpu_player)
